...
users = [
    {"id": 1, "full_name": "john", "email": "john123@gmail.com",
     "password": "adminpass"},
]
...
def home(request):
    ...
    return render(request, 'play_list_app/home.html', {"my_playlists": playlists})


def about(request):
    try:
        thankyou_next = Song.objects.create(track="thank u, next",
                                            artist="Ariana Grande",
                                            album="thank u, next",
                                            length="3:27", playlist_id=1)
        one_kiss_next = Song.objects.create(track="One Kiss, next",
                                            artist="Dua Lipa, Calvin Harris",
                                            album="One Kiss", length="3:34",
                                            playlist_id=1)
        better_now = Song.objects.create(track="Better Now",
                                         artist="Post malone",
                                         album="beerbongs & bentleys",
                                         length="3:51", playlist_id=1)
        the_middle = Song.objects.create(track="The Middle",
                                         artist="Grey,Marren Morris, ZEDD",
                                         album="The Middle", length="3:04",
                                         playlist_id=1)
        love_lies = Song.objects.create(track="Love Lies",
                                        artist="Normani, Khalid",
                                        album="Love Lies", length="3:21",
                                        playlist_id=2)
        rise = Song.objects.create(track="Rise",
                                   artist="Jack & Jack, Jonas Blue",
                                   album="Blue", length="3:14", playlist_id=2)

        car_playlist = Playlist.objects.create(name="Car Playlist",
                                               number_of_songs=4)
        coding_playlist = Playlist.objects.create(name="Coding Playlist",
                                                  number_of_songs=2)
...
def playlist(request, id):
    ...
    return render(request, 'play_list_app/songs.html',
...
def edit(request, id):
    ...
    if form.is_valid():
        ...
        return render(request, 'play_list_app/edit.html', {"form": form,
...
def signup(request):
    ...
    if form.is_valid():
        ...
        if (password != confirm_password):
            return render(request, 'play_list_app/signup.html', {"form": form,
...
def login(request):
    ...
    return render(request, 'play_list_app/login.html',
